...
export default function SummaryStats({ userName, refreshTrigger }: SummaryStatsProps) {
      ...
      try {
        ...
        if (data) {
          ...
          const prevWeekSum = data.reduce((sum, record) => {
            ...
            if (timestamp >= previousWeekStart && timestamp <= previousWeekEnd) {
          ...
          data.forEach(record => {
            ...
            if (timestamp >= eveningRange.start && timestamp <= eveningRange.end) {
  ...
  return (
    <div className="card">
      <h2 className="text-xl font-semibold text-gray-800 mb-4">Consumption Summary</h2>
        ...
        <div className="space-y-6">
          <div className="mb-6">
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-gray-700">Weekly Consumption</span>
              <span className={`text-sm font-medium ${stats.week > 14 ? 'text-red-600' : 'text-gray-700'}`}>
                {stats.week} / 14 units
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-100 rounded-lg p-3 text-center">
              <p className="text-sm text-gray-500 mb-1">Tonight</p>
              <p className="text-2xl font-semibold text-primary-700">{stats.evening}</p>
            ...
            <div className="bg-gray-100 rounded-lg p-3 text-center">
              ...
              <p className="text-2xl font-semibold text-primary-700">{stats.month}</p>
              <p className="text-xs text-gray-500">units</p>
            </div>
            
            <div className="bg-gray-100 rounded-lg p-3 text-center">
              <p className="text-sm text-gray-500 mb-1">Year</p>
              <p className="text-2xl font-semibold text-primary-700">{stats.year}</p>
              <p className="text-xs text-gray-500">units</p>
            </div>
          </div>
          
          <div className="pt-2 text-center text-sm text-gray-500">
            <p>Recommended limit: 14 units per week</p>
          </div>
        </div>
      )}
    </div>
